...
class EventEdit extends Component
{
    ...
    public function setTeamIntroBlock($text)
    {
        $team_intro_block = EventBlock::where([
            'event_version_id' => $this->event->active_version->id,
            'block_type' => self::TEAM_INTRO_BLOCK
        ])->first();
        if ($team_intro_block) {
            $team_intro_block->block_content = $text;
            $team_intro_block->update();
        } else {
            EventBlock::create([
                'event_version_id' => $this->event->active_version->id,
                'block_type' => self::TEAM_INTRO_BLOCK,
        ...
        session()->flash('success', 'Team intro Block set successfully.');
    }
