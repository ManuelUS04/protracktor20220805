<div x-cloak
    ...
    blockTextModal: null,
    showTitleModal: null,
    showSubtitleModal: null,
    showTeamIntroModal: null,
    textMode: null,
    text: null,
    ...
    edit_title: false,
    edit_subtitle: false,
    edit_team_intro: false,
    editing: false,
    showNewBlockModal: null,
    showEditBlockModal: null,
    block_modal_mode: null,
    block_name: null,
    ...
    x-init="
        // Background
        blockBgModal = new bootstrap.Modal(document.querySelector(`#block_bg_modal`), {backdrop: `static`});

        bgDropzone = new Dropzone(`#bg_dropzone`, {
            url: `{{ route('communication.event.setBlockBg', ['event_version' => $event->active_version]) }}`,
            paramName: `file`,
            maxFiles: 1,
            maxFilesize: 20, // MB
            addRemoveLinks: true,
        ...
        bgDropzone.on('complete', (file) => {
            blockBgModal.hide();
            Livewire.emit('refreshComponent');
        });

        // Logo
        blockLogoModal = new bootstrap.Modal(document.querySelector(`#block_logo_modal`), {backdrop: `static`});

        logoDropzone = new Dropzone(`#logo_dropzone`, {
            url: `{{ route('communication.event.setBlockLogo', ['event_version' => $event->active_version]) }}`,
            paramName: `file`,
            maxFiles: 1,
            maxFilesize: 10, // MB
            addRemoveLinks: true,
        ...
        logoDropzone.on('complete', (file) => {
            blockLogoModal.hide();
            Livewire.emit('refreshComponent');
        });

        blockTextModal = new bootstrap.Modal(document.querySelector(`#block_text_modal`), {backdrop: `static`});
        ...
        showTitleModal = function() {
            ...
            let callback = function (event) {
                if (text && editing) {
                    $wire.setTitleBlock(text);
                }
                text = null;
                editing = false;
                document.querySelector('#block_text_modal').removeEventListener('hidden.bs.modal', callback);
            };
            document.querySelector('#block_text_modal').addEventListener('hidden.bs.modal', callback);
        };

        showSubtitleModal = function() {
            blockTextModal.show();
            textMode = 'subtitle';
            let callback = function (event) {
                if (text && editing) {
                    $wire.setSubtitleBlock(text);
                }
                editing = false;
                text = null;
                document.querySelector('#block_text_modal').removeEventListener('hidden.bs.modal', callback);
        ...
        showTeamIntroModal = function() {
            blockTextModal.show();
            textMode = 'team_intro';
            let callback = function (event) {
                if (text && editing) {
                    $wire.setTeamIntroBlock(text);
                }
                editing = false;
                text = null;
                document.querySelector('#block_text_modal').removeEventListener('hidden.bs.modal', callback);
        ...
        newBlockModal = new bootstrap.Modal(document.querySelector(`#new_block_modal`), {backdrop: `static`});
        ...
        showNewBlockModal = function() {
            ...
            tinymce.get('editor_text').on('init', function(e) {
                e.target.setContent(``);
            });
            tinymce.get('editor_text').setContent(``);
            newBlockModal.show();
            block_name = '';
            block_text = '';
            let callback = function (event) {
                let block_text = tinymce.get(`editor_text`).getContent();
                block_text = `<div class='bg-white rounded my-10 px-15 py-20'>${block_text}</div>`;
                if (block_text && editing) {
                    $wire.createBlock(block_text, block_name);
                }
                editing = false;
                document.querySelector('#new_block_modal').removeEventListener('hidden.bs.modal', callback);
            };
            document.querySelector('#new_block_modal').addEventListener('hidden.bs.modal', callback);
        };

        showEditBlockModal = function(id, block_title) {
            tinymce.init({
                selector: '#editor_text',
                height: '500',
                menubar: false,
            ...
            let content = document.querySelector(`#content-` + id).innerHTML;
            tinymce.get('editor_text').on('init', function(e) {
                e.target.setContent(content); 
            });
            tinymce.get('editor_text').setContent(content); 
            let callback = function (event) {
                let block_text = tinymce.get(`editor_text`).getContent();
                if (block_text && editing) {
                    $wire.updateBlock(block_text, block_name, id);
                }
                editing = false;
                document.querySelector('#new_block_modal').removeEventListener('hidden.bs.modal', callback);
            };
        ...
        @this.on(`showModalForEdit`, () => {
            showEditBlockModal(@entangle('block_id_selected').initialValue, @entangle('block_title_selected').initialValue);
        });
    ">
    <script src="/assets/js/tinymce/tinymce.min.js" referrerpolicy="origin"></script>
    <div class="pb-10 d-flex flex-row justify-content-between">
        <span class="fs-2hx fw-bolder me-2 lh-1 ls-n2 mb-10">{{ $event->name }}</span>
        <div class="card-toolbar gap-5">
            <div class="dropdown">
                ...
                <ul class="dropdown-menu" aria-labelledby="dropdownMenuButton1">
                    ...
                    <li><a class="dropdown-item" href="#">Previous Revisions</a></li>
                    <li><a class="dropdown-item" href="#">Seller</a></li>
                    <li><a class="dropdown-item" href="#">Previous Revisions</a></li>
                </ul>
                <a class="btn btn-sm btn-secondary" href="{{ route('communication.timeline.create') }}">
    ...
    <div class="bg-primary px-10 py-12">
        <div>
            ...
            <div class="bg-white rounded my-10">
                    ...
                    <div class="pt-10 position-relative">
                        @if ($event->active_version->logo_block_image)
                            <img src="{{ $event->active_version->logo_block_image }}" alt="home" 
                                style="width: 5.5em; cursor: pointer"
                                @mouseover="edit_logo = true"
                                @mouseover.away="edit_logo = false" 
                                @click="blockLogoModal.show()">
                            <div x-show="edit_logo" 
                                class='position-absolute w-150px'
                                style="left: 50%; transform: translate(-45%, 0%);">
                    ...
                    <div class="py-4 position-relative">
                        @if ($event->active_version->title_block)
                            <h1 class="display-1"
                                ...
                                @mouseover="edit_title = true">
                                {{ $event->active_version->title_block }}
                            </h1>
                            <div x-show="edit_title" @mouseover.away="edit_title = false" 
                                class='text-center position-absolute w-150px'
                                style="left: 50%; transform: translate(-50%, 0%);">
                                Click to Change Title
                            </div>
                        @else
                            <h1 class="border border-primary border-dashed border-hover p-5"
                                @click="showTitleModal()" style="cursor: pointer;">
                                Set Title
                                <button class="btn btn-sm me-1 p-1" type="button">
                                    <i class="bi bi-pencil-fill fs-2x"></i>
                                </button>
                    ...
                    <div class="mb-15 position-relative">
                        @if ($event->active_version->subtitle_block)
                            <h6 class="display-6 fw-light"
                                style="cursor: pointer;"
                                @click="showSubtitleModal()"
                                @mouseover="edit_subtitle = true">
                                {{ $event->active_version->subtitle_block }}
                            </h6>
                            <div x-show="edit_subtitle" @mouseover.away="edit_subtitle = false" 
                                class='text-center position-absolute w-175px'
                                style="left: 50%; transform: translate(-50%, 0%);">
                                Click to Change Subtitle
                            </div>
                        @else
                            <h1 class="border border-primary border-dashed border-hover p-5"
                                @click="showSubtitleModal()" style="cursor: pointer;">
                                Set Subtitle
                                <button class="btn btn-sm me-1 p-1" type="button">
                                    <i class="bi bi-pencil-fill fs-2x"></i>
                                </button>
                ...
                <div class="position-relative">
                    @if ($event->active_version->bg_block_image)
                        <div x-show="edit_bg" @mouseover.away="edit_bg = false" 
                            class='text-center position-absolute w-200px'
                        ...
                        <img src="{{ $event->active_version->bg_block_image }}" alt="image-lg" class="img-fluid rounded-bottom"
                            style="cursor: pointer;"
                            @mouseover="edit_bg = true"
                            @click="blockBgModal.show()">
                    @else
                        <div class="text-center border py-10 rounded-bottom"
                            style="cursor: pointer;"
                            @click="blockBgModal.show()">
                            <h1>
                                Set Background
            ...
            <div wire:sortable="updateSort">
                @foreach ($event->active_version->text_blocks as $event_block)
                    <div class="position-relative" wire:sortable.item="{{ $event_block->id }}" wire:key="task-{{ $event_block->id }}">
                        <div style="cursor: pointer;"
                            @mouseover="edit_block[{{ $event_block->id }}] = true;" @mouseover.away="edit_block[{{ $event_block->id }}] = false"
                            wire:click="editBlock({{ $event_block->id }})">
                            <div x-show="edit_block[{{ $event_block->id }}]"
                                :class="{'d-flex': edit_block[{{ $event_block->id }}]}"
                                class='text-center position-absolute w-100 h-100 d-flex flex-column justify-content-center'>
                                <div class="p-5 text-end">
                                    <i class="bi bi-arrows-move fs-2x" wire:sortable.handle></i>
                                </div>
                                <div class="flex-fill d-flex flex-column justify-content-center">
                                    <h6 class="display-6">{{ $event_block->block_title }}</h6>
                                    Click to Edit Block
                                </div>
                            </div>
                            <div id="content-{{ $event_block->id }}">
            ...
            <div style="cursor: pointer;" class="card bg-secondary border border-primary border-dashed border-hover"
                @click="showNewBlockModal()">
                <div class="card-body">
            ...
            <div class="bg-white rounded my-10 px-15 pt-10 pb-15">
                <div class="d-flex justify-content-center my-15 position-relative">
                    @if ($event->active_version->team_intro_block)
                        <h6 class="display-5"
                            style="cursor: pointer;"
                            @click="showTeamIntroModal()"
                            @mouseover="edit_team_intro = true">
                            {{ $event->active_version->team_intro_block }}
                        </h6>
                        <div x-show="edit_team_intro" @mouseover.away="edit_team_intro = false" 
                            class='text-center position-absolute w-150px'
                            style="left: 50%; transform: translate(-50%, 140%);">
                            Click to Change Team intro
                        </div>
                    @else
                        <h6 class="border border-primary border-dashed border-hover p-5"
                            @click="showTeamIntroModal()" style="cursor: pointer;">
                            Set Team Intro
                            <button class="btn btn-sm me-1 p-1" type="button">
                                <i class="bi bi-pencil-fill fs-2x"></i>
                            </button>
    ...
    <!-- Modal -->
    <div class="modal fade" id="block_bg_modal" tabindex="-1">
    ...
    <!-- Modal -->
    <div class="modal fade" id="block_logo_modal" tabindex="-1">
        <div class="modal-dialog modal-lg">
            <div class="modal-content">
                <div class="modal-header">
                    <h2 class="modal-title me-5">
                ...
                <div class="modal-body">
                    <div class="dropzone" id="logo_dropzone">
                        <!--begin::Message-->
                        <div class="dz-message needsclick">
                            <!--begin::Icon-->
                            <i class="bi bi-file-earmark-arrow-up text-primary fs-3x"></i>
    ...
    <!-- Modal -->
    <div class="modal fade" id="new_block_modal" tabindex="-1">
        <div class="modal-dialog modal-lg">
            <form @submit.prevent="editing = true; newBlockModal.hide()">
                <div class="modal-content">
                    <div class="modal-header">
                        <h2 class="modal-title me-5">
                            Block
                    ...
                    <div class="modal-body">
                        <input type="text" class="form-control form-control-lg" placeholder="Block Title" required
                            x-model="block_name">
                        <ul class="nav nav-tabs nav-line-tabs mb-5 fs-6">
                            <li class="nav-item">
                                <a class="nav-link active" data-bs-toggle="tab" href="#tab_block_text">Text</a>
                            </li>
                            <li class="nav-item">
                                <a class="nav-link" data-bs-toggle="tab" href="#tab_block_audience">Audience</a>
                            </li>
                        </ul>

                        <div class="tab-content">
                            <div class="tab-pane fade show active" id="tab_block_text" role="tabpanel">
                                <textarea id="editor_text">
                                </textarea>
                            </div>
                            <div class="tab-pane fade" id="tab_block_audience" role="tabpanel">
                                <span>
                                    Select the audience that will utilize this section content
                                </span>
                                <div class="audience-checboxes m-10">
                                    @foreach ($audience as $key => $item)
                                        <div class="form-check mb-5">
                                            <input class="form-check-input" type="checkbox" id="audience_{{ $key}}" 
                                                wire:model.defer="audience.{{$key}}.checked"
    ...
    <!-- Modal -->
    <div class="modal fade" id="block_text_modal" tabindex="-1">
        <div class="modal-dialog modal-sm">
            <form @submit.prevent="editing = true; blockTextModal.hide()">
                <div class="modal-content">
                    <div class="modal-header">
                        <h2 class="modal-title me-5">
                            <span x-show="textMode === 'title'">Block Title</span>
                            <span x-show="textMode === 'subtitle'">Block Subtitle</span>
                            <span x-show="textMode === 'team_intro'">Block Team Intro</span>
                        </h2>
                        <button type="button" class="btn-close" data-bs-dismiss="modal" aria-label="Close"></button>
                    </div>
                    <div class="modal-body">
                        <input class="form-control" placeholder="Inset Text" type="text" x-model="text" required />
                    </div>
                    <div class="modal-footer">
                        <div class="d-flex flex-row justify-content-end mt-5 mb-5">
                            <button type="submit" class="btn btn-info m-2 p-5">
                                Update
                            </button>
                        </div>
                    </div>
