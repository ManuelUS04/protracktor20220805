                        ...
                        <div class="draggable">
                            ...
                            <div class="d-flex align-items-center justify-content-between bg-secondary rounded p-3 mb-7 border border-gray-900 draggable-handle">
                                ...
                                <div class="d-flex h-40px">
                                    @if ($event->sms)
                                    <button class="btn btn-sm me-1 p-1" type="button" @click="$wire.showSMSModal({{ $event->id }})" title="Set SMS">
                                        <i class="bi bi-phone-fill fs-2x"></i>
                                    </button>
                                    @endif
                                    @if ($event->preview)
                                    <a class="btn btn-sm me-1 p-1" href="{{ route('communication.event.show', $event)}}" title="Preview">
                                        <i class="bi bi-eye-fill fs-2x"></i>
                                    </a>
                                    @endif
                                    @if ($event->edit)
                                    <a class="btn btn-sm me-1 p-1" href="{{ route('communication.event.edit', $event)}}" title="Edit">
                                    ...
                                    <div class="draggable">
                                        ...
                                        <div class="d-flex align-items-center justify-content-between bg-secondary rounded p-3 mb-7 border border-gray-900 draggable-handle">
                                            ...
                                            <div class="d-flex h-40px">
                                                ...
                                                <button class="btn btn-sm me-1 p-1" type="button" @click="$wire.showSMSModal({{ $selected_event->id }})" title="Set SMS">
                                                    <i class="bi bi-phone-fill fs-2x"></i>
                                                </button>
                                                @endif
                                                @if ($selected_event->preview)
                                                <button class="btn btn-sm me-1 p-1" type="button" title="Preview">
                                                    <i class="bi bi-eye-fill fs-2x"></i>
                                                </button>
                                                @endif
                                                @if ($selected_event->edit)
                                                <a class="btn btn-sm me-1 p-1" type="button" href="{{ route('communication.event.edit', $event)}}" title="Edit">
                                                    <i class="bi bi-pencil-fill fs-2x"></i>
                                                </a>
                                                @endif
                                            </div>
